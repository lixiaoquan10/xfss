...
//                    xmlObjectStatus.nValue = element.attribute("ID").toInt();
                    ...
                    if (!query.exec())
                    {
                        qDebug() << "QueryLampSoftwareInfo Error:" << query.lastError().text();
                        return;
                    }
...
//                    xmlObjectStatus.nValue = element.attribute("ID").toInt();
                    ...
                    if (!query.exec())
                    {
                        qDebug() << "QueryDeviceSoftwareInfo Error:" << query.lastError().text();
                        return;
                    }
                    QSqlRecord record = query.record();
                    int fieldDistributionAddress = record.indexOf("address");
                ...
                else if(element.tagName() == "QueryDeviceRealtimeData")
                {
                    ...
                    if(!db.open())
                        return;
                    QSqlQuery query(db);
                    query.prepare(QString("SELECT id,canport,address from CanDeviceInfo"
                                                       "where id=%1"
                                                       ).arg(element.attribute("ID").toInt()));
                    if (!query.exec())
                    {
                        qDebug() << "QueryDeviceRealtimeData Error:" << query.lastError().text();
                        return;
                    }
                    QSqlRecord record = query.record();
                    int fieldDistributionAddress = record.indexOf("address");
