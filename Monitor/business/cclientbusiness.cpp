...
void CClientBusiness::XmlLayoutPage()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT ID,BackgroundColor,BackgroundPicture,PageWidth,PageHeight,Remark from LayoutPage"));
    if (!query.exec())
    {
        qDebug() << "XmlLayoutPage Error:" << query.lastError().text();
        return;
    }
...
void CClientBusiness::XmlLampCoordinate()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT c.id,c.lampinfoid,c.layoutpageid,c.cx,c.cy,c.width,c.height,c.angle from Coordinate c"));
    if (!query.exec())
    {
        qDebug() << "XmlLampCoordinate Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
...
void CClientBusiness::XmlFirePoint()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT f.id,f.device,f.loop,f.address,f.area,f.location from FirePoint f"));
    if (!query.exec())
    {
        qDebug() << "XmlFirePoint Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::XmlFirePointCoordinate()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT f.id,f.firepointid,f.layoutpageid,f.cx,f.cy,f.width,f.height from FirePointCoordinate f"));
    if (!query.exec())
    {
        qDebug() << "XmlFirePointCoordinate Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::XmlLampToFirePoint()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT f.id,f.lampid,f.firepointid from LampToFirePoint f"));
    if (!query.exec())
    {
        qDebug() << "XmlLampToFirePoint Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::XmlLampDefaultDirection()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT f.id,f.lampinfoid,f.lampdirection from LampDefaultDirection f"));
    if (!query.exec())
    {
        qDebug() << "XmlLampDefaultDirection Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::XmlLampControl(CMsgObjectStatus* xmlObjectStatus)
{
    ...
    querydevice.prepare(QString("SELECT l.id,l.channelinfoid,l.address,r.number from LampInfo l "
                                             "left join lamptype r on l.lamptypeid=r.id "
                                             "where l.id=%1"
                                       ).arg(lampInfoID));
    if (!querydevice.exec())
    {
        qDebug() << "querydevice Error:" << querydevice.lastError().text();
        return;
    }
    ...
    QSqlQuery query(db);
    query.prepare(QString("SELECT m.id,m.candeviceinfoid,m.loop,c.address from ChannelInfo m "
                                       "left join CanDeviceInfo c on m.candeviceinfoid=c.id "
                                       "where m.id=%1"
                                       ).arg(ChannelInfoID));
    if (!query.exec())
    {
        qDebug() << "XmlLampControl Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldLoopAddress = record.indexOf("loop");
...
void CClientBusiness::serverFirePoint()
{
    ...
    if (!query.exec())
    {
        qDebug() << "serverFirePoint Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::serverLampToFirePoint()
{
    ...
    if (!query.exec())
    {
        qDebug() << "serverLampToFirePoint Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::serverLampConnection()
{
    ...
    if(!db.open())
        return;
    QSqlQuery query(db);
    query.prepare(QString("SELECT f.id,f.StartLampID,f.Joint,f.EndLampID from LampConnection f"));
    if (!query.exec())
    {
        qDebug() << "serverLampConnection Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::serverEventList(CMsgObjectStatus* xmlObjectStatus)
{
    ...
    if (!query.exec())
    {
        qDebug() << "serverEventList Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
...
void CClientBusiness::serverPageInfo()
{
    ...
    if (!query.exec())
    {
        qDebug() << "serverPageInfo Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("ID");
...
void CClientBusiness::serverLampCoordinate()
{
    ...
    if (!query.exec())
    {
        qDebug() << "serverLampCoordinate Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldKeyId = record.indexOf("id");
...
void CClientBusiness::serverFirePointCoordinate()
{
    ...
    if (!query.exec())
    {
        qDebug() << "serverFirePointCoordinate Error:" << query.lastError().text();
        return;
    }
    QSqlRecord record = query.record();
    int fieldId = record.indexOf("id");
...
void CClientBusiness::DeviceResetStatus()
{
    ...
    for(int m=0; m < m_defaultdevices.count(); m++)
    {
        CDevice* device = m_defaultdevices.at(m);
        if(device)
        {
            ...
            exeCommand(NCT_DeviceDefaultDrection, msgObjectStatus);
        }
    }
    m_defaultdevices.clear();
}
        ...
        else if(CGlobal::instance()->m_lampEmergencyType & 0x01)
        {
            if(CGlobal::instance()->m_isOldLampEmergencyParam)
            {
                if(isDoubleDirection)
                {
                    ...
                    msgObjectStatus->nValue = direction;
                    exeCommand(NCT_DeviceDefaultDrection, msgObjectStatus);
                    m_defaultdevices.append(device);
...
void CClientBusiness::Emergency()
{
    ...
    if (!query.exec())
    {
        qDebug() << "Emergency Error:" << query.lastError().text();
        return;
    }
    //query.value(0)代表第0列
    //第0列:l.Address        灯具地址
    ...
    while(query.next())
    {
        ...
        int direction = 0;
        //双向灯和地埋灯除外,它们在方向控制时闪烁  //照明灯除外
        if(isDoubleLamp(lamptypeID) || isLightLamp(lamptypeID))
            continue;
...
//        device->setDeviceValue(DEVICE_VALUE_EMERGENCY,"应急");
        ...
        //闪烁或者可变状态灯
        if(CGlobal::instance()->m_TwinkleControl || isChangeLamp(lamptypeID) || device->isMAtype())
        {
            CMsgObjectStatus msgObjectStatus;
            msgObjectStatus.nCanportAdd = canport->canportAdd();
            msgObjectStatus.nDisID = distribution->distributionAddress();
            ...
            msgObjectStatus.nDeviceID = device->deviceAdd();
            //闪烁
            if(device->isMAtype() || CGlobal::instance()->m_lampEmergencyType == 0)
            {

            }
            else if(CGlobal::instance()->m_lampEmergencyType == 0)
            {
                //非双向灯全亮
                direction |= 0x07;
                //闪烁
    ...
    querylampcontrol.prepare(QString("SELECT l.LampDirection,r.Remark,r.Address,c.Loop,a.CanPort,a.Address,r.id,r.LampTypeID,r.IsReverse from EvacuationPathItem l "
